...
def main():
    ...
    with open('Data/t5-base_cts_fine-tune_chrf.csv') as file:
        csv_reader = csv.DictReader(file, delimiter=',')
        rows_keep = [row for row in csv_reader if len(row['Source'].split()) > 6]

    filtered_rows = compare_generations(rows_keep)

    with open("Results/t5-base/filtered_similarity_t5-base_len6.csv", "w", newline="") as f:
